    ...
    if(data.uid)
    {
        ...
        storage.get('sel_format', function(error, data) {
            ...
            if(data.sel_format && (user_id !== undefined))
            {
                ...
                let saved_d1 = db.ref(user_id+"/saved_formats/D9");
                saved_d1.once("value", function(snapshot) {
                    ...
                    saved_d1.child(sel_for+"/data").once("value", function(snapshot) {
                        snapshot.forEach(function (childSnapshot) {
                            ...
                            let inputNoOSP = childSnapshot.child("inputNoOSP").val();
                            let inputSalOff = childSnapshot.child("inputSalOff").val();
        ...
        $('#add_row').click(async () => {
            ...
            )
            {
                ...
                let inputNoOSP = $('#inputNoOSP').val();
                let inputSalOff = $('#inputSalOff').val();

                console.log("data");
                console.log(sel_for);

                let ref = db.ref(user_id+"/saved_formats/D9");
                    ...
                    ref.child(sel_for+"/data/"+last_row_cl).set(
                        {
                            ...
                            inputNoOSP: inputNoOSP,
                            inputSalOff: inputSalOff
                        }
                    );
        ...
        $('#print_preview').click(() => {
            ...
            win.loadURL(`file://${__dirname}/pr_d9.html`);
